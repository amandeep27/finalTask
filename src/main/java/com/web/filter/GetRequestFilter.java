...
public class GetRequestFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		if (queryString != null) {
			...
			if (userInfo != null) {
				if (!queryString.contains("&submit=getBook")) {
					if (queryString.contains("submit=getBookInfo") || queryString.contains("submit=removeBook")) {
						if (userInfo.getUserRole().equalsIgnoreCase(ADMINISTRATOR)) {
							chain.doFilter(req, res);
						} else if (userInfo.getUserRole().equalsIgnoreCase(USER)) {
