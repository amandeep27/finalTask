...
public class UserDaoImpl implements UserDao {
	...
	private static final String SELECT_QUERY = "select * from ul_info where ul_user_name = ?;";
	private static final String IF_VALID_USER = "select exists( select * from ul_info where ul_user_name = ?) as 'userExists';";
	...
	private static final String NAME = "ul_user_name";
	...
	private static final String INSERT_QUERY = "insert into ul_info(ul_name, ul_user_name, ul_pwd, ul_role, ul_lang_l_id) values(?,?,?,?,?);";
	...
	private static final String UPDATE_USER_INFO = "update ul_info set ul_name=?, ul_pwd=? where ul_id=?;";
	private static final String GET_USER_PWD = "select ul_pwd from ul_info where ul_id=?;";

	public UserInfo getUserInfoByName(SignInInfo signInInfo) throws DaoException {
		...
		try {
			...
			preparedStmt.setString(1, signInInfo.getName());
	...
	public boolean saveInfoInDB(SignUpInfo signUpInfo) throws DaoException {
		...
		try {
			...
			preparedStmt.setString(1, signUpInfo.getUserName());
			preparedStmt.setString(2, signUpInfo.getName());
	...
	public void editUserInformation(EditInfo info) throws DaoException {
		Connection connection = null;
		PreparedStatement preparedStmt = null;

		try {
			connection = connectionPool.getConnection();
			preparedStmt = connection.prepareStatement(UPDATE_USER_INFO);
			preparedStmt.setString(1, info.getName());
			preparedStmt.setString(2, info.getPwd());
			preparedStmt.setInt(3, info.getId());
			preparedStmt.executeUpdate();
		} catch (SQLException exception) {
			LOGGER.error("SQL Exception", exception);
			throw new DaoException(exception);
	...
	public String getUserPwd(int id) throws DaoException {
		...
		try {
			connection = connectionPool.getConnection();
			preparedStmt = connection.prepareStatement(GET_USER_PWD);
			preparedStmt.setInt(1, id);
			resultSet = preparedStmt.executeQuery();
			while (resultSet.next()) {
				originalPwd = resultSet.getString(PWD);
			}
		} catch (SQLException exception) {
			LOGGER.error("SQL Exception", exception);
			throw new DaoException(exception);
