...
public class BookDaoImpl implements BookDao {
	...
	public List<BookInfo> getBookInfo(GetBookInfo getBookInfo) throws DaoException {
		...
		try {
			...
			LOGGER.error("ConnectionPool Exception", exception);
			throw new DaoException(exception);
		} finally {
	...
	public void addbook(Book book) throws DaoException {
		...
		try {
			...
			LOGGER.info("author exists : " + authorExist);
			connectionPool.close(resultSet, preparedStmt);
			resultSet = null;
			preparedStmt = null;
			if (authorExist == 0) {
			...
			if (entryExist == 0) {
				...
				preparedStmt.setInt(2, bookTypeId);
				preparedStmt.executeUpdate();
				LOGGER.info("inserted in book info type table");
			}
			...
			if (entryExist != 1) {
				...
				preparedStmt.setInt(2, authorId);
				preparedStmt.executeUpdate();
				LOGGER.info("inserted in book info author table");
			}
	...
	public void removeBook(String langType, int bookId) throws DaoException {
		...
		try {
			...
			preparedStmt.executeUpdate();
			LOGGER.info("deleted from book_info_author");
			...
			preparedStmt.setInt(1, bookId);
			preparedStmt.executeUpdate();
			LOGGER.info("deleted from book_info_type");
			connectionPool.close(preparedStmt);
			preparedStmt = null;
			...
			preparedStmt.setInt(1, bookId);
			preparedStmt.executeUpdate();
			LOGGER.info("deleted from book_info");
	...
	public boolean updateBookInfo(BookInfo bookOldInfo, Book bookNewInfo) throws DaoException {
		LOGGER.info(bookNewInfo.getBookLanguage());
		...
		try {
			...
			LOGGER.info(getBookTypeId);
			LOGGER.info(bookNewInfo.getType());
			preparedStmt = null;
			preparedStmt = connection.prepareStatement(getBookTypeId);
			...
			if (newTypeId != bookOldInfo.getType().getTypeId()) {
				LOGGER.info("diff type encountered");
				resultSet = null;
				preparedStmt = null;
				...
				LOGGER.info(bookOldInfo.getType().getTypeId());
				connectionPool.close(resultSet, preparedStmt);
				preparedStmt = null;
				if (hasType != 0) {
